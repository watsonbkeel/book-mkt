...
Development uses isolated mocks, not the production Docker services. Real Debian Docker upgrade, SMTP/IMAP delivery and live configured model/search require separate operator authorization and staging checks. Do not relabel mock tests as production proof.
...
## Current contracts (1.3; source verification remains v3)
...
- New composition uses model-written complete subject/body and a versioned evidence brief, independently checked in all three modes. No required quote/pattern or fallback template. No unsupported facts or false Re:/Fwd: subject.
...
- Current implementation specification: docs/CODEX_BOOK_MKT_UPGRADE.md. Current acceptance: docs/ACCEPTANCE_1.3.md. Older review/evidence documents describe historical results only.
