...
Marketing model maximum 150 daily (failed marketing requests count). Classification, reply composition and reply review have **no daily model-call cap** and do not consume marketing quota; they remain recorded for usage visibility. Research plus extraction/continuations can use at most 50 of the 150 requests. Research entry requests additionally have a daily limit of 30. Existing model request timeouts, thread/identity/review checks and email sending limits are unchanged. No counters are reset during rollout. Fresh-install defaults remain conservative/off.
