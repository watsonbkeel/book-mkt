...
Every request, failed attempt, revision, review and Messages continuation counts against the existing budgets. A generation/tick chain is bounded to 100s and 10 model requests; each network timeout is capped by remaining time. Worker SIGALRM enforces wall time on the main thread; graceful stop prevents subsequent dispatch. No budget increase is made by a migration.
...
Protocol references: https://developers.openai.com/api/docs/guides/reasoning and https://platform.openai.com/docs/guides/reasoning (retrieval returned HTTP403 in this environment; no current model capability claim); https://platform.claude.com/docs/en/build-with-claude/adaptive-thinking (HTTP200 redirected to an unavailable-in-region page; current protocol content could not be verified). Payload shape is tested with captured mock requests; no live gateway, model ID or effort effectiveness was tested.
