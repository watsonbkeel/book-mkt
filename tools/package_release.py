...
def package(output,include_untracked=False):
    files=[Path(x) for x in subprocess.check_output(['git','ls-files','-z'],cwd=ROOT).decode().split('\0') if x]
    if include_untracked:
        files=sorted(set(files+[Path(x) for x in subprocess.check_output(['git','ls-files','--others','--exclude-standard','-z'],cwd=ROOT).decode().split('\0') if x]))
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('output',type=Path);p.add_argument('--include-untracked',action='store_true');a=p.parse_args();print(json.dumps(package(a.output,a.include_untracked)))
