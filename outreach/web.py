...
SETTING_GROUPS=[
 ...
 ('调度与预算',[('timezone','每日额度统计时区','text'),('daily_limit','首封每日上限（最多10）','number'),('gap_minutes','首封最小间隔分钟（至少61，默认70）','number'),('window_start','首封开始时刻','time'),('window_end','首封结束时刻','time'),('outbound_mode','邮件生成后的处理','mode'),('daily_reply_limit','自动/人工回复每日上限','number'),('reply_gap_minutes','回复之间最小间隔分钟','number'),('daily_thread_replies','每线程滚动24小时自动回复上限','number'),('max_thread_replies','每线程自动回复总上限','number'),('daily_api_calls','每日营销模型请求上限（回复不计入）','number'),('research_interval_minutes','自动研究间隔（分钟）','number'),('daily_research_calls','每日搜索研究请求上限','number'),('daily_fetches','每日来源页面核验上限','number'),('research_batch_size','一次研究最多候选数','number'),('queue_target','活跃候选池上限（待核、可起草及排队合计，最多2000）','number'),('domain_cooldown_days','相同业务域名首封冷却天数（至少365）','number'),('max_source_age_days','来源核验有效天数','number'),('retention_days','邮件正文保留天数','number')])]
