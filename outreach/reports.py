...
## Operational state
- Generation contract: 3; database schema: {store.one('SELECT version FROM schema_version')['version']}
...
def schedule_snapshot(store,config,now=None):
    ...
    used=store.one("SELECT count(*) n FROM api_usage WHERE kind IN ('llm','research') AND NOT (kind='llm' AND purpose IN ('classification','reply','reply_review')) AND created_at>=? AND created_at<?",(a,b))['n']
